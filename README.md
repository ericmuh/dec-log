# Decision Log - Lesson 6: APIs, Flask-Marshmallow, Flask-RESTful

This branch is a standalone lesson focused on moving from basic Flask APIs with jsonify to extension-based API development using Flask-RESTful and Flask-Marshmallow.

## What Was Built

- Basic API routes using Flask + jsonify (in-memory data):
  - GET /api/basic/decisions
  - GET /api/basic/decisions/<id>
  - POST /api/basic/decisions
- Extension-based API setup:
  - Flask-SQLAlchemy for Decision model and database persistence
  - Flask-Migrate for migrations
  - Flask-RESTful resources under /api/v1
  - Flask-Marshmallow schema for serialization and validation
- RESTful endpoints (database-backed):
...
- Reading request JSON from request.get_json()
- Structuring APIs with Resource classes
- Marshmallow schema-based validation and serialization
- SQLAlchemy model persistence for API responses
...
3. Run migration setup (first time):
   ...
   flask --app run db migrate -m "create decision table"
...
Basic jsonify API:
```bash
curl http://127.0.0.1:5000/api/basic/decisions
```

RESTful API:
```bash
curl -X POST http://127.0.0.1:5000/api/v1/decisions \
  -H "Content-Type: application/json" \
  -d '{"title":"Adopt code reviews","reason":"Improve quality","confidence_level":4}'
```
